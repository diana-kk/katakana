...
//이미지
var g_melonn = Paper.image('img/g_melonn.jpg', 0, 0, 100, 105).toDefs();
...
bottomArea.text(97, 87, 'マ').click(handlerA).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'メ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_36_me.wav');
  audio1.play();
}
...
g_melonn.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['メ', 'ロン', '멜론']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_36_meronn.wav');
  audio2.play();
}
