...
//이미지
var g_koora = Paper.image('img/g_koora.jpg', 0, 0, 85, 95).toDefs();
...
bottomArea.text(171, 270, 'コ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_12_ko.wav');
  audio1.play();
}
...
g_koora.use().transform('t140, 417').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(175, 550, ['コ', 'ーラ', '콜라']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_12_koora.wav');
  audio2.play();
}
