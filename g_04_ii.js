...
//이미지
var g_igirisu = Paper.image('img/g_igirisu.jpg', 0, 0, 110, 80).toDefs();
...
bottomArea.text(97, 87, 'ア').click(handlerA).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'イ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_04_ii.wav');
  audio1.play();
}
...
g_igirisu.use().transform('t135, 425').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['イ', 'ギリス', '영국']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_04_igirisu.wav');
  audio2.play();
}
