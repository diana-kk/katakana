...
//이미지
var g_nitto = Paper.image('img/g_nitto.jpg', 0, 0, 100, 100).toDefs();
...
bottomArea.text(171, 270, 'ニ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_24_ni.wav');
  audio1.play();
}
...
g_nitto.use().transform('t140 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['ニ', 'ット', '니트']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_24_nitto.wav');
  audio2.play();
}
