...
//이미지
var g_suisu = Paper.image('img/g_suisu.jpg', 0, 0, 115, 90).toDefs();
...
bottomArea.text(171, 270, 'ス').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_15_su.wav');
  audio1.play();
}
...
g_suisu.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['ス', 'イス', '스위스']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_15_suisu.wav');
  audio2.play();
}
