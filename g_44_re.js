...
//이미지
var g_retaa = Paper.image('img/g_retaa.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'レ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_44_re.wav');
  audio1.play();
}
...
g_retaa.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['レ', 'ター', '편지']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_44_reta.wav');
  audio2.play();
}
