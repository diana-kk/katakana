...
//이미지
var g_razio = Paper.image('img/g_razio.jpg', 0, 0, 100, 100).toDefs();
...
bottomArea.text(171, 270, 'ラ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_41_ra.wav');
  audio1.play();
}
...
g_razio.use().transform('t130 410').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(170, 550, ['ラ', 'ジオ', '라디오']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_41_razio.wav');
  audio2.play();
}
