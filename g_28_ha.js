...
//이미지
var g_hamu = Paper.image('img/g_hamu.jpg', 0, 0, 130, 95).toDefs();
...
topArea.text(22, 337, 'ハ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(150, 87, 'ヒ').click(handlerI).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(203, 87, 'フ').click(handlerU).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerU() {
  location.replace('g_30_hu.html');
}
...
bottomArea.text(256, 87, 'ヘ').click(handlerE).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerE() {
  location.replace('g_31_he.html');
}
...
bottomArea.text(309, 87, 'ホ').click(handlerO).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerO() {
  location.replace('g_32_ho.html');
}
...
bottomArea.text(171, 270, 'ハ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_28_ha.wav');
  audio1.play();
}
...
g_hamu.use().transform('t130 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['ハ', 'ム', '햄']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_28_hamu.wav');
  audio2.play();
}
