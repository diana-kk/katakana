...
//이미지
var g_ribonn = Paper.image('img/g_ribonn.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(309, 87, 'ロ').click(handlerO).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerO() {
  location.replace('g_45_ro.html');
}
...
bottomArea.text(171, 270, 'リ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_42_ri.wav');
  audio1.play();
}
...
g_ribonn.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(186, 550, ['リ', 'ボン', '리본']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_42_ribonn.wav');
  audio2.play();
}
