...
//이미지
var g_hoteru = Paper.image('img/g_hoteru.jpg', 0, 0, 95, 95).toDefs();
...
bottomArea.text(171, 270, 'ホ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_32_ho.wav');
  audio1.play();
}
...
g_hoteru.use().transform('t140 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['ホ', 'テル', '호텔']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_32_hoteru.wav');
  audio2.play();
}
