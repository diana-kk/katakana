...
//이미지
var g_naasu = Paper.image('img/g_naasu.jpg', 0, 0, 60, 105).toDefs();
...
topArea.text(22, 287, 'ナ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(150, 87, 'ニ').click(handlerI).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(203, 87, 'ヌ').click(handlerU).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerU() {
  location.replace('g_25_nu.html');
}
...
bottomArea.text(256, 87, 'ネ').click(handlerE).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerE() {
  location.replace('g_26_ne.html');
}
...
bottomArea.text(309, 87, 'ノ').click(handlerO).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerO() {
  location.replace('g_27_no.html');
}
...
bottomArea.text(171, 270, 'ナ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_23_na.wav');
  audio1.play();
}
...
g_naasu.use().transform('t145 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['ナ', 'ース', '간호사']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_23_nasu.wav');
  audio2.play();
}
