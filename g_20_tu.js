...
//이미지
var g_tuna = Paper.image('img/g_tuna.jpg', 0, 0, 130, 85).toDefs();
...
bottomArea.text(171, 270, 'ツ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_20_tu.wav');
  audio1.play();
}
...
g_tuna.use().transform('t130 425').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(190, 550, ['ツ', 'ナ', '참치']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_20_tuna.wav');
  audio2.play();
}
