...
//이미지
var g_miruku = Paper.image('img/g_miruku.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ミ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_34_mi.wav');
  audio1.play();
}
...
g_miruku.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['ミ', 'ルク', '우유']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_34_miruku.wav');
  audio2.play();
}
