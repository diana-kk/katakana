...
//이미지
var g_yooyoo = Paper.image('img/g_yooyoo.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ヨ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_40_yo.wav');
  audio1.play();
}
...
g_yooyoo.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 550, ['ヨ', 'ーヨー', '요요']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_40_yoyo.wav');
  audio2.play();
}
