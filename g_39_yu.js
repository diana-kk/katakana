...
//이미지
var g_yunihoomu = Paper.image('img/g_yunihoomu.jpg', 0, 0, 80, 110).toDefs();
...
// 단
bottomArea.circle(156, 80, 20).click(hadlerYa).attr({
  'fill': '#FCF5D4',
  'cursor': 'pointer'
});
bottomArea.text(148, 87, 'ヤ').click(hadlerYa).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.circle(209, 80, 20).attr({
  'fill': '#FCF5D4',
  'stroke': '#FFB2AF',
  'stroke-width': 3
});
bottomArea.text(201, 87, 'ユ').attr({
  'font-size': 20,
  'font-weight': 'bold'
});
...
bottomArea.text(171, 270, 'ユ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_39_yu.wav');
  audio1.play();
}
...
g_yunihoomu.use().transform('t140 410').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(132, 550, ['ユ', 'ニホーム', '유니폼']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_39_yunihomu.wav');
  audio2.play();
}
