...
//이미지
var g_nuudoru = Paper.image('img/g_nuudoru.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ヌ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_25_nu.wav');
  audio1.play();
}
...
g_nuudoru.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 550, ['ヌ', 'ードル', '누들']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_25_nudoru.wav');
  audio2.play();
}
