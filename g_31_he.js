...
//이미지
var g_heapinn = Paper.image('img/g_heapinn.jpg', 0, 0, 125, 80).toDefs();
...
bottomArea.text(97, 87, 'ハ').click(handlerA).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'ヘ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_31_he.wav');
  audio1.play();
}
...
g_heapinn.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(150, 550, ['ヘ', 'アピン', '머리핀']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_31_heapinn.wav');
  audio2.play();
}
