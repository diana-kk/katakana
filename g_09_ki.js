...
//이미지
var g_kii = Paper.image('img/g_kii.jpg', 0, 0, 115, 105).toDefs();
...
bottomArea.text(171, 270, 'キ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_09_ki.wav');
  audio1.play();
}
...
g_kii.use().transform('t130, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(200, 550, ['キ', 'ー', '열쇠']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_09_ki_voca.wav');
  audio2.play();
}
