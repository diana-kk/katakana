...
//이미지
var g_usagi = Paper.image('img/g_usagi.png', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'ウ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_05_uu.wav');
  audio1.play();
}
...
g_usagi.use().transform('t135, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['ウ', 'サギ', '토끼']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_05_usagi.wav');
  audio2.play();
}
