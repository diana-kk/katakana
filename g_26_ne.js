...
//이미지
var g_nekutai = Paper.image('img/g_nekutai.jpg', 0, 0, 60, 100).toDefs();
...
bottomArea.text(97, 87, 'ナ').click(handlerA).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'ネ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_26_ne.wav');
  audio1.play();
}
...
g_nekutai.use().transform('t145 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(150, 550, ['ネ', 'クタイ', '넥타이']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_26_nekutai.wav');
  audio2.play();
}
