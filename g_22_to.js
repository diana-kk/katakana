...
//이미지
var g_tomato = Paper.image('img/g_tomato.jpg', 0, 0, 95, 90).toDefs();
...
bottomArea.text(171, 270, 'ト').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_22_to.wav');
  audio1.play();
}
...
g_tomato.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['ト', 'マト', '토마토']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_22_tomato.wav');
  audio2.play();
}
