...
//이미지
var g_maiku = Paper.image('img/g_maiku.jpg', 0, 0, 100, 85).toDefs();
...
topArea.text(22, 387, 'マ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(150, 87, 'ミ').click(handlerI).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(203, 87, 'ム').click(handlerU).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerU() {
  location.replace('g_35_mu.html');
}
...
bottomArea.text(256, 87, 'メ').click(handlerE).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerE() {
  location.replace('g_36_me.html');
}
...
bottomArea.text(309, 87, 'モ').click(handlerO).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerO() {
  location.replace('g_37_mo.html');
}
...
bottomArea.text(171, 270, 'マ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_33_ma.wav');
  audio1.play();
}
...
g_maiku.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(175, 550, ['マ', 'イク', '마이크']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_33_maiku.wav');
  audio2.play();
}
