...
//이미지
var g_oilu = Paper.image('img/g_oilu.png', 0, 0, 80, 100).toDefs();
...
bottomArea.text(171, 270, 'オ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_07_oo.wav');
  audio1.play();
}
...
g_oilu.use().transform('t145, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['オ', 'イル', '기름']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_07_oiru.wav');
  audio2.play();
}
