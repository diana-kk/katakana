...
//이미지
var g_keeki = Paper.image('img/g_keeki.jpg', 0, 0, 110, 95).toDefs();
...
bottomArea.text(171, 270, 'ケ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_11_ke.wav');
  audio1.play();
}
...
g_keeki.use().transform('t140, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['ケ', 'ーキ', '케이크']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_11_keiki.wav');
  audio2.play();
}
