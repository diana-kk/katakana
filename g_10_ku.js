...
//이미지
var g_kurasu = Paper.image('img/g_kurasu.jpg', 0, 0, 110, 95).toDefs();
...
bottomArea.text(97, 87, 'カ').click(handlerA).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(171, 270, 'ク').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_10_ku.wav');
  audio1.play();
}
...
g_kurasu.use().transform('t140, 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 550, ['ク', 'ラス', '반/학급']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_10_kurasu.wav');
  audio2.play();
}
