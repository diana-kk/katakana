...
//이미지
var g_hiitaa = Paper.image('img/g_hiitaa.jpg', 0, 0, 80, 95).toDefs();
...
bottomArea.text(171, 270, 'ヒ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_29_hi.wav');
  audio1.play();
}
...
g_hiitaa.use().transform('t140 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['ヒ', 'ーター', '히터']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_29_hita.wav');
  audio2.play();
}
