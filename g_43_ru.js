...
//이미지
var g_rubii = Paper.image('img/g_rubii.jpg', 0, 0, 90, 90).toDefs();
...
bottomArea.text(171, 270, 'ル').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_43_ru.wav');
  audio1.play();
}
...
g_rubii.use().transform('t130 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(186, 550, ['ル', 'ビー', '루비']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_43_rubi.wav');
  audio2.play();
}
