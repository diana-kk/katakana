...
//이미지
var g_kamera = Paper.image('img/g_kamera.jpg', 0, 0, 110, 90).toDefs();
...
topArea.text(22, 137, 'カ').click(hanlderLine).attr({
  'font-size': 20,
  'font-weight': 'bold',
  'cursor': 'pointer'
});
...
bottomArea.text(150, 87, 'キ').click(handlerI).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(203, 87, 'ク').click(handlerU).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerU() {
  location.replace('g_10_ku.html');
}
...
bottomArea.text(256, 87, 'ケ').click(handlerE).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerE() {
  location.replace('g_11_ke.html');
}
...
bottomArea.text(309, 87, 'コ').click(handlerO).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
function handlerO() {
  location.replace('g_12_ko.html');
}
...
bottomArea.text(171, 270, 'カ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_08_ka.wav');
  audio1.play();
}
...
g_kamera.use().transform('t130, 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['カ', 'メラ', '카메라']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_08_kamera.wav');
  audio2.play();
}
