...
//이미지
var g_muusu = Paper.image('img/g_muusu.jpg', 0, 0, 70, 105).toDefs();
...
bottomArea.text(171, 270, 'ム').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_35_mu.wav');
  audio1.play();
}
...
g_muusu.use().transform('t130 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(185, 550, ['ム', 'ース', '무스']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_35_musu.wav');
  audio2.play();
}
