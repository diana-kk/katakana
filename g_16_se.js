...
//이미지
var g_seetaa = Paper.image('img/g_seetaa.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'セ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_16_se.wav');
  audio1.play();
}
...
g_seetaa.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(150, 550, ['セ', 'ーター', '스웨터']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_16_seta.wav');
  audio2.play();
}
