...
//이미지
var g_huruutu = Paper.image('img/g_huruutu.jpg', 0, 0, 110, 90).toDefs();
...
bottomArea.text(171, 270, 'フ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_30_hu.wav');
  audio1.play();
}
...
g_huruutu.use().transform('t140 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(160, 550, ['フ', 'ルーツ', '과일']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_30_hurutu.wav');
  audio2.play();
}
