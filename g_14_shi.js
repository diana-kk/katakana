...
//이미지
var g_siisoo = Paper.image('img/g_siisoo.jpg', 0, 0, 120, 80).toDefs();
...
bottomArea.text(171, 270, 'シ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_14_shi.wav');
  audio1.play();
}
...
g_siisoo.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(165, 550, ['シ', 'ーソー', '시소']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_14_siso.wav');
  audio2.play();
}
