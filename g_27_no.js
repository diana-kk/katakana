...
//이미지
var g_nootoo = Paper.image('img/g_nootoo.jpg', 0, 0, 100, 90).toDefs();
...
bottomArea.text(171, 270, 'ノ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_27_no.wav');
  audio1.play();
}
...
g_nootoo.use().transform('t135 420').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(180, 550, ['ノ', 'ート', '노트']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_27_noto.wav');
  audio2.play();
}
