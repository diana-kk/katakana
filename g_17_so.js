...
//이미지
var g_sooseeji = Paper.image('img/g_sooseeji.jpg', 0, 0, 115, 100).toDefs();
...
bottomArea.text(97, 87, 'サ').click(handlerA).attr({
  'font-size': 20,
  'fill': 'gray',
  'cursor': 'pointer'
});
...
bottomArea.text(167, 270, 'ソ').click(playAud01).attr({
  'font-size': 100,
  'cursor': 'pointer'
});

function playAud01() {
  var audio1 = new Audio('aud/aud_g_17_so.wav');
  audio1.play();
}
...
g_sooseeji.use().transform('t135 415').click(playAud02).appendTo(bottomArea).attr({
  'cursor': 'pointer'
});

var t12 = bottomArea.text(127, 550, ['ソ', 'ーセージ', '소시지']).click(playAud02).attr({
  'font-size': 25,
  'cursor': 'pointer'
});
...
function playAud02() {
  var audio2 = new Audio('aud/aud_g_17_soseji.wav');
  audio2.play();
}
